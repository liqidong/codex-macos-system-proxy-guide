...
macOS 快捷指令可以换颜色和符号，但如果你想用自己的图片，最稳的是做一个 Automator App。
...
6. 替换 `REPO_DIR`、`HTTP_PROXY_URL`、`HTTPS_PROXY_URL`、`ALL_PROXY_URL`。
7. 保存为 `Codex via Proxy.app`。

换图标：

```text
Finder -> 选中 Codex via Proxy.app -> Command + I
```
...
- 背景留一点边距，不要把主体贴到边缘。
- 图标名建议包含用途，比如 `codex-via-proxy.icns`。
